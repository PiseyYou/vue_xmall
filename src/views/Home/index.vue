      ...
      <section class="w mt30 clearfix" v-if="item.type === 2">
        <m-shelf :title="item.name">
          <div slot="content" class="hot">
            <!-- {{ 123456 }} -->
            <mall-goods v-for="(item, index) in item.panelContents" :key="index" :goods="item">
              <!-- {{ o.productImageBig }} -->
            </mall-goods>
          </div>
        </m-shelf>
      ...
      <section class="w mt30 clearfix" v-if="item.type === 3">
        <m-shelf :title="item.name">
          <div slot="content" class="floors">
            <template v-for="(item, index) in item.panelContents">
              <div class="imgbanner" :key="index" v-if="item.type == 2 || item.type === 3">
                <img :src="item.picUrl" alt="" />
              </div>
            </template>
            <template v-for="(item, index) in item.panelContents">
              <mall-goods :key="index" :goods="item" v-if="item.type === 0"></mall-goods>
            </template>
          </div>
        </m-shelf>
      </section>
